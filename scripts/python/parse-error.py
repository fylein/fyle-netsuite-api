...
errors_count = Error.objects.filter(type='NETSUITE_ERROR', is_resolved=False).count()
print(errors_count)
page_size = 200
count = 0
for offset in range(0, errors_count, page_size):
    limit = offset + page_size
    paginated_errors = Error.objects.filter(type='NETSUITE_ERROR', is_resolved=False).order_by('id')[offset:limit]
    for error in paginated_errors:
        ...
        configuration = Configuration.objects.get(workspace_id=error.workspace_id)
        if fund_source == 'PERSONAL':
            configuration_export_type = Configuration.objects.get(workspace_id=error.workspace_id).reimbursable_expenses_object
        else:
            configuration_export_type = Configuration.objects.get(workspace_id=error.workspace_id).corporate_credit_card_expenses_object
