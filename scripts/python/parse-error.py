...
for offset in range(0, errors_count, page_size):
    ...
    for error in paginated_errors:
        configuration = Configuration.objects.filter(workspace_id=error.workspace_id).first()
        message = error.error_detail
        fund_source = error.expense_group.fund_source
        task_log = TaskLog.objects.filter(expense_group_id=error.expense_group.id).first()
        if fund_source == "PERSONAL":
            ...
            error_dict, article_link = error_matcher(
                message=message,
                export_type=configuration_export_type,
        ...
        if message:
            ...
            task_log.detail['message'] = message
            error.save()
            count += 1
