...
@pytest.mark.django_db()
def test_export_exception_handling_bill(mocker, db, add_netsuite_credentials, add_fyle_credentials):
    """
    Test export bill with attachment upload exception
    """
    mocker.patch('apps.netsuite.tasks.update_complete_expenses', side_effect=Exception('Update expenses failed'))
    mocker.patch('apps.netsuite.tasks.post_accounting_export_summary', side_effect=Exception('Post summary failed'))
    mocker.patch('apps.netsuite.tasks.publish_to_rabbitmq', side_effect=Exception('Publish failed'))
    mocker.patch('apps.netsuite.connector.NetSuiteConnector.post_bill', return_value=data['creation_response'])
    mocker.patch('apps.netsuite.actions.patch_request', return_value=None)

    expense_groups = list(ExpenseGroup.objects.filter(workspace_id=1, fund_source='PERSONAL'))
    TaskLog.objects.filter(expense_group__in=expense_groups).delete()
    ...
    configuration = Configuration.objects.get(workspace_id=1)
    configuration.is_attachment_upload_enabled = True
    configuration.save()
...
@pytest.mark.django_db()
def test_export_exception_handling_journal_entry(mocker, db, add_netsuite_credentials, add_fyle_credentials):
    """
    Test export journal entry with attachment upload exception
    """
    mocker.patch('apps.netsuite.tasks.update_complete_expenses', side_effect=Exception('Update expenses failed'))
    mocker.patch('apps.netsuite.tasks.post_accounting_export_summary', side_effect=Exception('Post summary failed'))
    mocker.patch('apps.netsuite.tasks.publish_to_rabbitmq', side_effect=Exception('Publish failed'))
    mocker.patch('apps.netsuite.connector.NetSuiteConnector.post_journal_entry', return_value=data['creation_response'])
    mocker.patch('apps.netsuite.actions.patch_request', return_value=None)
...
@pytest.mark.django_db()
def test_export_expense_report_attachment_upload_exception(mocker, db, add_netsuite_credentials, add_fyle_credentials):
    """
    Test export expense report with attachment upload exception
    """
    mocker.patch('apps.netsuite.tasks.publish_to_rabbitmq', side_effect=Exception('Publish to RabbitMQ failed'))
    mock_response = {'internalId': '12345', 'externalId': 'test-123', 'type': 'expensereport'}
    mocker.patch('apps.netsuite.connector.NetSuiteConnector.post_expense_report', return_value=mock_response)
    mocker.patch('apps.netsuite.tasks.update_complete_expenses', return_value=None)
    mocker.patch('apps.netsuite.tasks.post_accounting_export_summary', return_value=None)
    mocker.patch('apps.netsuite.tasks.update_expense_and_post_summary', return_value=None)
    mocker.patch('apps.netsuite.actions.patch_request', return_value=None)
